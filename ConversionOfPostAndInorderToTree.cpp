//Given Post Order and In Order Traversal of a tree in form of array, construct the tree.     
...
public:
    ...
    Node *buildTreeUtil(int in[], int post[], int inStart, int inEnd, int postStart, int postEnd) {
        ...
        root->left = buildTreeUtil(in, post, inStart, rootIndex - 1, postStart, postStart + (rootIndex - inStart) - 1);
        root->right = buildTreeUtil(in, post, rootIndex + 1, inEnd, postStart + (rootIndex - inStart), postEnd - 1);
